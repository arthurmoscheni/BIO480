...
# def get_contact_responses(sweep, cols, fs=20000, ici_threshold=0.200, 
#                           do_filter=False, max_slope_thresh=2.0):
#     raw_vm = sweep[cols['vm']]
#     contacts = sweep[cols['contacts']]
    
#     if raw_vm is None or len(raw_vm) == 0: return []
#     if contacts is None: return []
#     try:
#         if np.isscalar(contacts) and pd.isna(contacts): return []
#     except: pass

#     contacts = np.array(contacts)
#     if contacts.ndim == 0 or len(contacts) == 0: return []
...
def get_contact_responses(
    ...
    for i in range(len(contacts)):
        ...
        snippet_clean = remove_aps_interpolation(
            raw_snippet, fs=fs, slope_thresh=20,
            win_ms=6.0, voltage_safety_thresh=-0.040
        )
...
#             #     ici_threshold=config.ICI_THRESHOLD,
...
#     return pd.DataFrame(cell_metrics), trace_storage


# Note: get_contact_responses function remains unchanged from previous step
